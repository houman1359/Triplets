...
function [vine,logp] = mixedvinefit(x,type,iscont,tl,refine,cop_fam)
...
%   both margins and copula families. TYPE can be one of
%    'c-vine' for the canonical vine or
%    'd-vine' for the D-vine.
...
switch lower(type)
    ...
    case 'd-vine'
        fitdtree(1,d);
    otherwise
        error(['mixedvinefit: Unknown vine type "' type '"']);
end
...
if treelevel > truncation
    ...
    if ~isempty(newvine.families{i,j})
        families = {newvine.families{i,j}};
    else
