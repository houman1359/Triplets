function [fcon,f,y_vector1,y_vector2]=TwoD_marginalize(copul,vine_stim,DIM,N,Tn,B,tvec)
...
%%%%% TN = the time point which we compute the 2D densities
            ...
            y2=vine_stim.margins{DIM(2)}.ker;
            y_vector2 =linspace(min(y2),max(y2)+eps,B(2));%unique(y);
            ...
            ff=reshape(f_points_train,B(1),B(2),numel(TN));
            f=squeeze(sum(ff,3));
            ffn=sum(f);
