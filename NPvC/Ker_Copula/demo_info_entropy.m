...
% function demo_info_entropy(N,rp,MOD,knots)
...
% cd '/n/data2/hms/neurobio/harvey/Houman/Copula_project/Info_Entropy/slurm'
% try

system('module load gcc/6.2.0');
system('module load python/2.7.12');
...
%     SIGMA = [5 -4; -4 5];
%     mu=[0 0];
%     X{1}(:,[1 2]) = mvnrnd(mu,SIGMA,N);
%     sx=1;
%     sy=1;
%
%
%     SIGMA = [sx^1 ro*sx*sy;ro*sx*sy sy^2];
%     X{2}(:,[1 2]) = mvnrnd(mu,SIGMA,N);
%     Y(:,[1 2]) = mvnrnd(mu,SIGMA,10000);
%
%     I_theoretical=0.5 * log2(sx^2 * sy^2/det(SIGMA));
%
%     a=X{2};
%     clear X Y
%     X=a;
        ...
        try
            ...
            for MODEL=MOD
                ...
                if MODEL<7
                    ...
                    EX= (exist(['/n/data2/hms/neurobio/harvey/Houman/Copula_project/Info_Entropy/infos/NRevisionSep_nNew_info_MOD_',num2str(MODEL),'_N_',num2str(N),'_rep_',num2str(rep),'_knots_',num2str(knots),'.mat'],'file')==0);
                else
                ...
                if  EX==1%exist(['/n/data2/hms/neurobio/harvey/Houman/Copula_project/Info_Entropy/infos//nNew_info_MOD_',num2str(MODEL),'_N_',num2str(N),'_rep_',num2str(rep),'_knots_',num2str(knots),'.mat'],'file')==0
                    ...
                    [info(1),stderr_1,in_1] = kernelvineinfo(vine,copula_LL1,20000,15); %%% this was 50 iter for revisions
                    ...
                    %                     [info(1),stderr_1,in_1] = kernelvineinfo_OlderThanAugust(vine,copula_LL1,[1],[],5e-3,20000,'copcop',vineest,40)
                    %                     [info(2),stderr_2,in_2] = kernelvineinfo_OlderThanAugust(vine,copula_LL2,[1],[],5e-3,20000,'copcop',vineest,40)
                    %                     [info(3),stderr_3,in_3] = kernelvineinfo_OlderThanAugust(vineest,copula_LL1,[0.5 0.5],[],5e-3,20000,'par',[],40)
...
%                          this is the 200 rep for revision
...
%                          this is the 1000 rep for revision
...
%                          this is the 100 rep for revision to test more
%                          iterations
                        save(['/n/data2/hms/neurobio/harvey/Houman/Copula_project/Info_Entropy/infos/NRevisionSep_nNew_info_MOD_',num2str(MODEL),'_N_',num2str(N),'_rep_',num2str(rep),'_knots_',num2str(knots),'.mat'])
...
for rep=1:3:100
    for NN= 10%5:13%13%11:12%5:13
        for MOD=[1 2 3 4 5 7]%[1 2 3 4 5 7]%1:6
            for knots= [10 30 50 100 150 200]
                N=2^NN;
                EX=0;
                for repn=(rep-1)*9+1:rep*9
                    ...
                    if exist(['/n/data2/hms/neurobio/harvey/Houman/Copula_project/Info_Entropy/infos/NRevisionSep_nNew_info_MOD_',num2str(MOD),'_N_',num2str(N),'_rep_',num2str(repn),'_knots_',num2str(knots),'.mat'],'file')==0
                ...
                if EX~=0%exist(['/n/data2/hms/neurobio/harvey/Houman/Copula_project/Info_Entropy/infos/info_MOD_',num2str(MOD),'_N_',num2str(N),'_rep_',num2str(repn),'_knots_',num2str(knots),'.mat'],'file')==0
                    if NN<11
                        system(['sbatch -p short -n 1 -t 0-1:0 --mem-per-cpu=1G -o /n/data2/hms/neurobio/harvey/Houman/TEMP/SLURM/%N_%j.out --wrap="matlab -nodisplay -r  \"run /home/hs258/Codes_Folder/Houman_Git/Ker_Copula/demo_info_entropy(',num2str(N),',',num2str(rep),',',num2str(MOD),',',num2str(knots),')\""'])
                    else
                        %                         mmm=mmm+1;
                        %                         c=parcluster;
